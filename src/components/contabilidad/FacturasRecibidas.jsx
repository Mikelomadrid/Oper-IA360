// FacturasRecibidas.jsx
import React, { useState, useEffect, useMemo } from 'react';
import { Receipt, RefreshCw, Search, Calendar, Building2, AlertCircle, ChevronDown, ChevronUp } from 'lucide-react';
import { getFacturasRecibidas, formatters } from '@/services/contasimpleService';
...
export default function FacturasRecibidas({ navigate }) {
  ...
  const loadFacturas = async () => {
    ...
    setError(null);
    try {
      const response = await getFacturasRecibidas(filters.fromDate);
      if (response.success && response.data) {
        const data = response.data.expenses?.data || response.data.data || response.data || [];
  ...
  const filteredFacturas = useMemo(() => {
    return facturas.filter((f) => {
      if (filters.search) {
        ...
        if (!f.number?.toLowerCase().includes(s) && !f.issuer?.organization?.toLowerCase().includes(s) && !f.issuer?.nif?.toLowerCase().includes(s)) return false;
      }
  ...
  if (loading) {
    return <div className="flex items-center justify-center h-64"><RefreshCw className="w-8 h-8 animate-spin text-primary" /><span className="ml-3 text-muted-foreground">Cargando facturas recibidas...</span></div>;
  }
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 p-4 bg-card border rounded-xl">
        <div>
          <h2 className="text-lg font-semibold flex items-center gap-2"><Receipt className="w-5 h-5 text-red-500" />Facturas Recibidas</h2>
          <p className="text-sm text-muted-foreground">Gastos y compras desde Contasimple</p>
        </div>
        <div className="flex items-center gap-4">
          <div className="text-right">
            <p className="text-sm text-muted-foreground">Total Gastos</p>
            <p className="text-xl font-bold text-red-600">{formatters.currency(totals.total)}</p>
          </div>
      ...
      <div className="flex flex-wrap gap-3 p-4 bg-muted/30 rounded-xl">
        <div className="flex-1 min-w-[200px] relative">
          ...
          <input type="text" placeholder="Buscar por nº, proveedor o NIF..." value={filters.search} onChange={(e) => setFilters({ ...filters, search: e.target.value })} className="w-full pl-10 pr-4 py-2 bg-background border rounded-lg text-sm" />
        </div>
      ...
      <div className="bg-card border rounded-xl overflow-hidden">
        ...
        {filteredFacturas.length === 0 ? (
          <div className="p-8 text-center text-muted-foreground">No se encontraron facturas recibidas</div>
        ) : (
              ...
              <div key={f.id}>
                <div className="grid grid-cols-12 gap-4 p-4 items-center hover:bg-muted/30 cursor-pointer" onClick={() => setExpandedId(expandedId === f.id ? null : f.id)}>
                  <div className="col-span-6 md:col-span-2">
                    <span className="font-medium text-red-600">{f.number}</span>
                  ...
                  <div className="hidden md:block col-span-4">
                    <p className="font-medium truncate">{f.issuer?.organization || '-'}</p>
                    <p className="text-xs text-muted-foreground">{f.issuer?.nif}</p>
                  </div>
                  ...
                  <div className="px-4 pb-4 bg-muted/20">
                    <div className="p-4 border rounded-lg bg-background space-y-4">
                      <div className="flex items-start gap-3">
                        ...
                        <div>
                          <p className="font-medium">{f.issuer?.organization}</p>
                          <p className="text-sm text-muted-foreground">{f.issuer?.nif} · {f.issuer?.address}, {f.issuer?.city}</p>
                        </div>
                      </div>
                      {f.lines?.length > 0 && (
                        <div>
      ...
      <div className="flex justify-end">
        <div className="bg-card border rounded-xl p-4 text-sm">
          <div className="grid grid-cols-2 gap-x-8 gap-y-1">
            ...
            <span className="text-muted-foreground">IVA soportado:</span><span className="text-right">{formatters.currency(totals.iva)}</span>
            <span className="font-medium pt-1 border-t">Total gastos:</span><span className="text-right font-bold text-red-600 pt-1 border-t">{formatters.currency(totals.total)}</span>
          </div>
        </div>
      </div>
    </div>
