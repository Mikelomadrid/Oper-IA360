...
import { format, startOfMonth, endOfMonth, subMonths, parseISO, isWithinInterval, isSameMonth } from 'date-fns';
...
const Dashboard = () => {
  ...
  const [period, setPeriod] = useState(12); // Default view period
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      // Always fetch 12 months of data to support switching views without refetching
      const oneYearAgo = subMonths(now, 12).toISOString();
      ...
      const {
        data: historicalData
      } = await supabase.from('datos_historicos').select('*').gte('fecha', oneYearAgo).order('fecha', {
        ascending: true
      });

      // ==========================================
      // 2. CARGAR DATOS REALES (LIVE - MES ACTUAL)
      // ==========================================
      ...
      } = await supabase.from('gastos_generales').select('importe, fecha').gte('fecha', oneYearAgo);
      const {
        data: unassignedExpenses
      } = await supabase.from('gastos').select('total_con_iva, fecha_emision, concepto').is('proyecto_id', null).gte('fecha_emision', oneYearAgo);
      const {
        ...
        `).is('proyecto_id', null).gte('hora_entrada', oneYearAgo).not('hora_salida', 'is', null);
      ...
      // --- CÁLCULO DE GRÁFICA HÍBRIDA (HISTÓRICO + REAL) ---
      ...
      for (let i = 11; i >= 0; i--) {
        ...
        if (manualData) {
          ...
          // -> USAR CÁLCULO REAL (ERP) PARA MESES RECIENTES
  ...
  // Derive visible data based on selected period
  const visibleChartData = chartData.slice(chartData.length - period);
      ...
      <div className="grid grid-cols-1 gap-4 md:gap-6">
        <Card className="col-span-1 shadow-sm">
          <CardHeader className="p-4 md:p-6 flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div>
              <CardTitle className="text-lg md:text-xl">Finanzas últimos {period} meses (Facturado)</CardTitle>
              <p className="text-xs md:text-sm text-muted-foreground">Evolución real de obras finalizadas y gastos de empresa.</p>
            </div>
            <div className="flex items-center gap-1 bg-muted/40 p-1 rounded-lg self-start md:self-auto">
                <Button variant={period === 3 ? "secondary" : "ghost"} size="sm" onClick={() => setPeriod(3)} className={`h-7 text-xs px-3 ${period === 3 ? 'bg-background shadow-sm text-foreground font-semibold' : 'text-muted-foreground'}`}>
                    3 meses
                </Button>
                <Button variant={period === 6 ? "secondary" : "ghost"} size="sm" onClick={() => setPeriod(6)} className={`h-7 text-xs px-3 ${period === 6 ? 'bg-background shadow-sm text-foreground font-semibold' : 'text-muted-foreground'}`}>
                    6 meses
                </Button>
                <Button variant={period === 12 ? "secondary" : "ghost"} size="sm" onClick={() => setPeriod(12)} className={`h-7 text-xs px-3 ${period === 12 ? 'bg-background shadow-sm text-foreground font-semibold' : 'text-muted-foreground'}`}>
                    12 meses
                </Button>
            </div>
          </CardHeader>
          <CardContent className="h-[300px] md:h-[400px] p-2 md:p-6 pt-0">
            <ResponsiveContainer width="100%" height="100%">
              <ComposedChart data={visibleChartData} margin={{
              top: 20,
              right: 10,
              left: -10,
              bottom: 5
